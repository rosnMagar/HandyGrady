...
# Assumes GEMINI_API_KEY is in your environmentQ
YOUR_API_KEY = os.environ.get('GEMINI_API_KEY')


def grade_answer_gemini(problem_images, answer_images, grading_standards, scoring_difficulty, output_folder="app/corrected_images", model_name='gemini-1.5-flash'):
    """
    ...
    try:
        ...
        final_full_score = 0 # To store the full score for the test
        ...
        for i, answer_img in enumerate(answer_imgs):
            ...
            - Provide a "full_score" inside "page_results" should be the maximum score of single problem.
            ...
            *   **If the scoring difficulty is 5:** The grading is not hard. To achieve full score, the student needs to demonstrate a general understanding of the topic. The answer needs to be reasonably related to the key points, single mistakes or missing can be ignored. It's easy to get high score even the answer is partial correct.
            ...
            *   **If the scoring difficulty is between 2 and 9:** The grading falls on a spectrum between these two extremes. Higher numbers mean stricter grading, and lower numbers mean more lenient grading. Use your best judgement to assign the score.
            ...
            Respond in JSON format with a list of question results and image modification instructions.
             "full_score" inside "page_results" should be the Full score of single problem.
            ...
            try:
                try:
                    ...
                    for question_result in page_results:
                        ...
                        if score is None or analysis is None or full_score is None:
                            print(f"Warning: Missing 'score', 'analysis' or 'full_score' for a question. Skipping this question.")
                            continue
                    ...
                    if response.text.strip() == "0":
                        all_scores.append(0)
                        full_score_list.append(0) # Append 0 so sum still works
                        all_analyses.append({
        ...
        Keep the response concise and helpful. Mention the strongest and weakest areas based on the individual question analyses from all pages.
        """
